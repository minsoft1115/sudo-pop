...
**알려진 구멍.** `BeginAuthentication` 이 자식을 기다리며 블로킹하는 동안
`CancelAuthentication` 이 처리되지 않는다 — zbus 블로킹 API 는 메서드를 직렬로 돌린다.
지금은 자식의 30초 백스톱이 대신 닫아 준다. **4단계에서 async 로 바꾸며 푼다.**
