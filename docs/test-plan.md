...
> **Tier 1·2 완료** (2026-08-19): 신규 유닛테스트 36개(lib 24→60). 리팩터는 전부
> 순수 함수 추출로 동작 변화 없음. #8 은 플레이크를 피해 유닛으로 대체.
...
- **6. `helper.rs` `split_tag` [완료]** (3개) — 태그/본문 분리, 공백 하나만 버림
- **7. `prompt.rs` 재시도 루프 [완료]** (4개) — `run_attempts` 추출, mock `authenticate`
  클로저로 "성공/취소 즉시 정지, 오답 후 재시도, 3회 캡, 오답 사이 `Wrong`" 검증
- **8. M3(취소된 대기 요청) [완료 — 유닛으로 대체]** (2개) — scenarios 로 하면 큐된 요청의
  취소를 실제 polkitd 로 재현해야 해 타이밍 플레이크가 된다("억지"). 대신 M3 의 핵심
...
- **10. `pam.rs` 스택 읽기 [완료]** (8개 + `fingerprint.rs` 5개 + `attempts.rs` 1개 + scenarios 6)
  — `auth_stack_names_with` 가 메모리 안의 PAM 디렉터리로 stock Arch(`include system-auth`)·
