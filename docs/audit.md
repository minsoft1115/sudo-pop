...
- **L1. [고침]** `KNOWN_AGENTS`에 `mate-polkit` 추가(`src/init.rs`). "3순위 활성 user
  유닛" 감지는 여전히 미구현 — omarchy.polkit(1순위)·프로세스명(2순위)이 실사용을 덮으므로
  후순위로 남긴다.
...
- **L3. [열림]** CJK 폰트 8MB 상한(`font.rs`)으로 현지화(한글) 메시지 깨질 수 있음.
  CJK 폴백 번들은 무겁고 §7·§9 가 후순위로 인지 중인 갭이라 이번엔 손대지 않음.
- **L4. [고침]** 요청별 상세 로깅을 `SUDO_POP_DEBUG` 뒤로 넣었다(`agent.rs`·`main.rs`).
  프로덕션 journal 에는 거절(REJECTED)·오류만 남는다. scenarios.sh 가 보는 REFUSED
  줄은 항상 남긴다.
- **L5. [열림/안전측]** `polkitd` 초기 소유자 획득과 `NameOwnerChanged` 구독 사이 좁은
  창(`main.rs`). fail-closed(진짜 폴킷을 잠깐 거절)라 안전 측 실패이고, 문서도 재시작
  실측 미완으로 인정 중이라 그대로 둔다.
