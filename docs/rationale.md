...
초기 상태는 **`Subject` 의 필드로** 창에 실어 보내고, 오답으로 재시도할 때는 `ToUi::Attempts`
채널 메시지로 실시간 갱신한다. 두 줄로 나뉘어 있어 `Wrong` 과 잔여 횟수가 서로의 슬롯을 침범하지
않는다.
...
### 22-8. F-6 — 외부 명령을 PATH 로 찾지 않는다

리뷰의 F-6 은 "위험 낮음" 으로 적었는데, 근거가 틀렸다. 자식이 물려받는 것은 시스템 매니저의
PATH 가 아니라 사용자 매니저의 것이고, Omarchy 는 거기에 `/usr/share/omarchy/bin`, mise shims,
`~/.local/bin` 을 `/usr/bin` 앞에 둔다. 같은 uid 의 아무 프로세스나 `~/.local/bin/faillock`
을 두면 다음 창부터 우리 자식이 그것을 실행했다. 비밀번호는 닿지 않는다 — 받는 것은 사용자
이름, 돌려주는 것은 표 텍스트 — 그러나 이 자식은 바로 그 이웃을 상대로 dumpable=0 과 mlock
을 거는 프로세스라, 이웃이 바이너리를 고르게 두는 것은 맞지 않았다.

세 가지를 바꿨다 (`src/attempts.rs`).

- **`date` 는 없앴다.** faillock 의 로컬 시각 문자열을 유닉스 초로 바꾸는 데 `date -d` 를 띄우고
  있었다. 표준 라이브러리에는 달력이 없지만 이미 의존하는 `libc` 에 `strptime` 과 `mktime` 이
  있다. `tm_isdst = -1` 이면 서머타임도 C 라이브러리가 판단한다 — `date` 가 하던 것과 같은
  계산이고, 하드닝된 자식에서 나가는 fork 가 하나 준다. 시험은 `localtime_r` + `strftime` 으로
  만든 문자열이 같은 초로 돌아오는지 보므로 어느 시간대에서도 성립한다.
- **`faillock` 은 절대 경로다.** `/usr/bin/faillock`, 없으면 `/usr/sbin/faillock` (Arch 는 둘 다
  있다). 둘 다 없으면 전처럼 줄도 거절도 없다. 오답 뒤에는 `pending` 에 비밀번호가 보관돼
  있을 수 있으므로 stdin 을 null 로 준다 (§22-4 와 같은 습관; 쿠키 파이프는 이미 EOF 라 실제로
  새던 것은 없다).
- **가짜 faillock 의 문은 디버그 빌드에만 있다.** `SUDO_POP_FAILLOCK_BIN` 은 `helper.rs` 의
  `SUDO_POP_HELPER_BIN` 과 같은 `cfg(debug_assertions)` 아래 있다. 릴리스에는 그 문자열조차
  없다. 시나리오 6 은 debug 바이너리로 가짜 tally 를 넘기고, 그 전에 릴리스 바이너리를 `grep`
  해 문자열이 없는 것과 debug 에는 있는 것을 둘 다 확인한다 — 12번이 가짜 헬퍼를 넘기는 것과
  같은 문이다.

보안 원칙과의 관계는 이렇다. 디버그 전용 오버라이드는 이미 헬퍼 경로에 허용된 것이고, 최악의
경우(디버그 빌드를 에이전트로 깔고 변수를 심는 것)에도 가짜가 할 수 있는 일은 숫자 왜곡·잠긴
척하기·창이 뜨는 시점 관찰뿐이다 — 지금까지 PATH 로 이미 가능하던 것이다. 릴리스는 좁아지고
디버그는 같은 자리에 머문다.
...
읽어 5·25·25 가 나오는지 본다. 실제 polkit 이 넘긴 subject 로 읽는 경로는 시나리오 3 이
`SYSTEMD_BUS_TIMEOUT=5`·`=120` run0 으로 본다 — 5초 출발, run0 이 포기한 직후 창이 닫히는 것,
25초 상한. 시나리오 스크립트는 머리에서 그 변수를 `unset` 한다. 개발 셸에 실험 값이 남아
있으면 이제 그것이 subject 의 environ 으로 들어가 "25초에 붙어 있다" 를 깨기 때문이다.
...
증거와 선택지를 모은 것은 [`agent-sudo-no-tty.md`](agent-sudo-no-tty.md).
