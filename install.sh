...
# under old/ and is not what this builds.
...
remove_by_hand() {
  ...
  if [ -f "$hypr" ] && grep -q -- '-- sudo-pop:begin' "$hypr"; then
    if grep -q -- '-- sudo-pop:end' "$hypr"; then
      ...
      say "removed the window rules from $hypr"
    else
...
do_uninstall() {
  ...
  if [ -n "$bin" ] && [ -x "$bin" ]; then
    # --uninit before rm: the binary is what knows where its files went, and it
    # is also what stops the agent.
    ...
    systemctl --user disable --now sudo-pop-agent.service 2>/dev/null || true
  fi

  [ "$uninit_ok" -eq 1 ] || remove_by_hand
...
other tools use it. Whatever polkit agent was there before takes over again;
on Omarchy that is:
...
# running — the agent usually is — cannot fail with ETXTBSY.
...
To remove it:
  ...
  # or, from a checkout:  ./install.sh --uninstall
DONE
