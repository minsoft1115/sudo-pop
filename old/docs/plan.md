...
| `unlock_time` | 120초 | **600초** | 잠긴 뒤 이만큼 지나면 풀린다 |
| `passwd_tries` | 10 | **3** | sudo 가 askpass 를 재호출하는 최대 횟수 |

**즉 이 머신에서는 15분 창 안에 실패 10건이면 잠긴다.** 창이 넓으므로 소비를
아끼는 것이 중요하다. 근거와 각 값의 출처는 `rationale.md` §7-1.

**숫자를 코드에 박지 말 것.** `deny` 와 `unlock_time` 은 실행 시점에 읽는다(§4-4c).
반면 아래 두 상수는 이 머신 값을 전제로 고른 것이므로, 상류 기본값 환경에서의
동작을 알고 있어야 한다:

| 상수 | 이 머신 | `passwd_tries=3` / `deny=3` 인 환경 |
|---|---|---|
| `MAX_ATTEMPTS = 3` | 10 → 3 으로 축소 | sudo 상한과 같아져 **아무것도 바꾸지 않는다** (무해) |
| `WARN_BELOW = 4` | 남은 예산 3 이하일 때만 경고 | 실패 0건에서도 `remaining=3` 이라 **항상 경고가 뜬다** |

둘 다 안전한 방향의 열화이므로 지금은 그대로 둔다. 경고가 상시 노출되는 것이
문제가 되면 `WARN_BELOW` 를 `deny` 에 비례시키는 것이 정공법이다.
...
- **탈출구**: `/usr/bin/sudo` **하나만** 안내한다. 바이너리가 사라지고 alias 만
  남았을 때의 복구 경로이며, 절대 경로만이 alias·셸 함수·PATH 선점을 전부 우회한다.
  `\sudo` 와 `command sudo` 를 함께 안내하면 각각의 한계를 설명하는 문단이 따라붙어
  복구 안내가 길어진다. 한계는 `rationale.md` §11-1 에 기록.
