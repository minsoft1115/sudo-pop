...
`no_screen_share` 가 화면 공유·녹화에서 창을 빼낸다. 스크린샷 도구도 같은
프로토콜을 쓰므로 창은 검은 사각형으로 찍힌다 — 이 README 맨 위의 사진은
`~/.config/minsoft1115/hypr/sudo-pop.lua` 에서 그 룰을 잠시 끄고 찍은 것이다.

### faillock — 취소도 실패 1건

sudo-pop 이 아니라 PAM 의 동작이다. 팝업이 뜬 시점에는 이미 인증 대화가 시작된
상태라, ESC 는 비밀번호를 틀린 것과 똑같이 1건을 소비한다. 실패가 연달아 쌓이면
PAM 이 계정을 일정 시간 잠근다.

기준값은 각자의 PAM 설정에 달려 있다 — sudo-pop 이 `deny` 와 `unlock_time` 을
실행 시점에 읽어 따라간다. 그 위에서:

- **`sudo` 명령 1회당 팝업은 최대 3회.** sudo 혼자서는 `passwd_tries` 만큼
  재시도하며 예산을 다 쓸 수 있다
- 남은 실패 예산이 얼마 없으면 **창에 경고**를 띄운다
- **이미 잠긴 상태면 팝업 자체를 띄우지 않는다**
...
| **나빠지는 것 하나** | 피싱. 터미널 프롬프트는 적어도 방금 명령을 친 자리에 뜨는데 팝업은 그것을 포기하고, 똑같이 생긴 창은 아무 권한 없이 그릴 수 있다. 맨 윗줄의 명령 표시가 그에 대한 대응이다 — 단서이지 보증은 아니다. |

실제로 지키는 것은 부주의로 인한 유출이다.

- 크래시해도 비밀번호가 코어 덤프에 남지 않는다
- 버퍼를 RAM 에 고정해 스왑·하이버네이션 이미지로 나가지 않는다
- 화면 공유·녹화에 창이 잡히지 않는다
- 로그·명령행·환경변수 어디에도 남지 않는다

전부 추정이 아니라 측정한 것이다 — `docs/rationale.md` §6.
