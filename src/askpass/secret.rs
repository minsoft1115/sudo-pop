//! The password buffer and the channel that carries it to sudo.
//!
//! sudo reads the first line of our stdout and treats it as the password, which
//! makes fd 1 a secret channel rather than an output stream. Two rules follow.
//!
//! First, nothing else may ever write there — a wgpu warning or a stray
//! `println!` would be sent to sudo as the password. So the real fd 1 is put
//! aside at startup and the descriptor is replaced with /dev/null, making
//! accidental writes structurally impossible rather than merely discouraged.
//!
//! Second, a failure must write *nothing at all*. An empty line reads as an
//! empty password, which sudo treats as wrong and retries up to `passwd_tries`
//! times — ten on this system, exactly the faillock `deny` threshold. Silence
//! makes sudo give up after one attempt instead.
